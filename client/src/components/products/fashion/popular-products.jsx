          ...
          <SwiperSlide key={item.id} className="tp-category-item-2 p-relative z-index-1 text-center">
            <div className="tp-category-thumb-2">
              <Link to={`/product-details/${item.id}`}>
                <img src={item.img} alt="product-img" width={224} height={260} />
              </Link>
            </div>
            <div className="tp-category-content-2">
              <span>From ${item.price}</span>
              <h3 className="tp-category-title-2">
                <Link to={`/product-details/${item.id}`}>{item.title.substring(0, 15)}</Link>
              </h3>
              <div className="tp-category-btn-2">
                {cart_products.some((prd) => prd.id === item.id) ? (
                  <Link to="/cart" className="tp-btn tp-btn-border cursor-pointer">
                    View Cart
                  </Link>
                ) : (
