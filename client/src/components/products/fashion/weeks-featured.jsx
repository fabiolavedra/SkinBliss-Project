...
const WeeksFeatured = () => {
  const { data: products, isError, isLoading } = useGetSuggestedProductsQuery({ type: 'fashion', query: `featured=true` });
            ...
            <SwiperSlide key={item.id} className="tp-featured-item white-bg p-relative z-index-1">
              ...
              <div className="tp-featured-content">
                ...
                <div className="tp-featured-price-wrapper">
                  {discount > 0 ? (
                    <>
                      ...
                      <span className="tp-featured-price old-price"> $ {(Number(price) - (Number(price) * Number(discount)) / 100).toFixed(2)}</span>
                    </>
                  ) : (
                    <span className="tp-featured-price new-price">${price}</span>
                  )}
                ...
                <div className="tp-featured-btn">
                  <Link to={`/product-details/${id}`} className="tp-btn tp-btn-border tp-btn-border-sm">
