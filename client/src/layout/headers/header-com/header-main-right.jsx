    ...
    <div className="tp-header-main-right d-flex align-items-center justify-content-end">
        ...
        <div className="d-flex align-items-center">
          <div className="tp-header-login-icon">
            <span>
              {userInfo?.imageURL ? (
                <Link to="/profile">
                  <img src={userInfo.imageURL} alt="user img" width={35} height={35} />
                </Link>
              ) : userInfo?.name ? (
                <Link to="/profile">
          ...
          <div className="tp-header-login-content d-none d-xl-block">
            ...
            <div className="tp-header-login-title">
              {!userInfo?.name && <Link to="/login">Sign In</Link>}
              {userInfo?.name && <Link to="/profile">Your Account</Link>}
            </div>
          </div>
        </div>
      </div>
      <div className="tp-header-action d-flex align-items-center ml-50">
        <div className="tp-header-action-item d-none d-lg-block">
          <Link to="/compare" className="tp-header-action-btn">
            <Compare />
          </Link>
        </div>
        <div className="tp-header-action-item d-none d-lg-block">
